...
def broadcast_database(message):
	...
	if len(CONNECTION_LIST) > 2:
		try:
			...
			print('Message sent to east coast')
		except:
			print('Cannot send message')
...
def dependency_check(message):
	...
	# database = int(message[2])
	# if DATABASE_TIME[database] == int(message[1]):
	# 	return True
	# else:
	# 	return False
...
if __name__ == '__main__':
	...
	try:
		...
		sock_21.connect(('localhost', 5001))
		sock_21.send(pickle.dumps('Database 5002'))
		print('Connect to Database 5001')
		CONNECTION_LIST.append(sock_21)
		# broadcast_database('Hello from 5002, west coast')
	except:
		print('Cannot connect Database 5001')

	# Connect to other Database
	try:
		sock_20 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		sock_20.connect(('localhost', 5000))
		sock_20.send(pickle.dumps('Database 5002'))
		print('Connect to Database 5000')
		CONNECTION_LIST.append(sock_20)
		# broadcast_database('Hello from 5002, west coast')
	except:
		print('Cannot connect Database 5000')

	while 1:

		socket_list = CONNECTION_LIST
						...
						if data[0] == 'D':
							...
							if data[0] != 'H':
								...
								if dependency_check(data[0]):
									print('Dependency check pass, update database and time')
									commit(data[1])
									print(DATABASE_TIME)
									DATABASE_TIME[5002] += 1
							...
							if data == 'Client':
								...
								if message[0] == 'write':
									...
									# new add
									DEPENDENCY[client] = [[message[2], DATABASE_TIME[5002], 5002]]
									...
									# DEPENDENCY[client] = [message[1], DATABASE_TIME[5002], DICTIONARY[message[1]]['From']]
									# new add
